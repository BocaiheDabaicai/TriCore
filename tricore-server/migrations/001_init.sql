-- TriCore 001: Initial schema — Sales, OA, Inventory
-- 18 tables, indexes, triggers
...
-- users: 商户(merchant) + 客户(customer) + 业务员(salesperson)
CREATE TABLE users (
...
-- products: shared between sales catalog and inventory stock
...
CREATE INDEX idx_bundle_sales_salesperson_id ON bundle_sales(salesperson_id);
CREATE TRIGGER trg_bundle_sales_updated_at BEFORE UPDATE ON bundle_sales
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX idx_return_orders_order_id ON return_orders(order_id);
CREATE TRIGGER trg_return_orders_updated_at BEFORE UPDATE ON return_orders
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX idx_workflow_forms_creator_id ON workflow_forms(creator_id);
CREATE INDEX idx_workflow_forms_status ON workflow_forms(status);
CREATE INDEX idx_workflow_forms_form_data ON workflow_forms USING GIN(form_data);
CREATE TRIGGER trg_workflow_forms_updated_at BEFORE UPDATE ON workflow_forms
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX idx_workflow_steps_reviewer_id ON workflow_steps(reviewer_id);
...
CREATE INDEX idx_workflow_archives_workflow_id ON workflow_archives(workflow_id);
...
CREATE INDEX idx_stock_in_records_order_id ON stock_in_records(order_id);
CREATE INDEX idx_stock_in_records_warehouse_id ON stock_in_records(warehouse_id);
CREATE TRIGGER trg_stock_in_records_updated_at BEFORE UPDATE ON stock_in_records
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX idx_stock_out_records_order_id ON stock_out_records(order_id);
CREATE INDEX idx_stock_out_records_warehouse_id ON stock_out_records(warehouse_id);
CREATE TRIGGER trg_stock_out_records_updated_at BEFORE UPDATE ON stock_out_records
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX idx_issues_related ON issues(related_type, related_id);
