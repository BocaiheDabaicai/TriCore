...
- 四个页面：
  ...
  - 回顾页：列表头检索行（搜索框 + 高级筛选开关）——开关开启后左侧出现筛选面板（类型/标签两组标签按钮），关闭时重置面板内筛选
  - 寻文页：文献站点按类型汇总，点击跳转去找论文
  - 详情页：Markdown 渲染；底部一行 [论文文件名（精读，点击打开）] … [删除（确认弹窗）] [修改]
...
│   ├── src/components/      # Sidebar（hover 图标轨道）/ MarkdownEditor（CodeMirror 6）/ ToastHost
